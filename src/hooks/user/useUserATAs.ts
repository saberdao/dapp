    ...
    return useQuery({
        ...
        queryFn: async (): Promise<AssociatedTokenAccount[]> => {
            ...
            const tokenAmounts = (
                                    ...
                                    try {
                                        return {
                                            ...
                                            balance: new TokenAmount(
                                                chunk[i].mint,
                                                (
                                                    item?.data as ParsedAccountData
                                        ...
                                        if (
                                            !ignoreWrap &&
                                            (chunk[i].mint.address === RAW_SOL_MINT.toString() ||
                                                chunk[i].mint.address ===
                                                    WRAPPED_SOL[network].address)
                                        ) {
                ...
                .flat()
                .filter(
                    (x): x is { key: PublicKey; balance: TokenAmount; isInitialized: boolean } =>
                        !!x,
                );
            return tokenAmounts;
        },
